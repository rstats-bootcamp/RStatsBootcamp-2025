...
overall_mean <- mean(seed$pct)
overall_sd <- sd(seed$pct)

# Create boxplot
boxplot(pct ~ treatment, data = seed,
        main = "Percentage Growth by Treatment",
        xlab = "Treatment Type",
...
abline(h = overall_mean, col = "red", lty = 2, lwd = 2)

# Add horizontal lines for mean +/- 1 SD
abline(h = overall_mean + overall_sd, col = "gray", lty = 3, lwd = 1)
abline(h = overall_mean - overall_sd, col = "gray", lty = 3, lwd = 1)

# Add legend
legend("topright", 
       legend = c("Overall Mean", "Mean ± 1 SD"),
       col = c("red", "gray"),
       lty = c(2, 3),
       lwd = c(2, 1))
```

This code creates a comprehensive boxplot visualization that:
1. Shows the distribution of growth percentages across different treatments
2. Includes a red dashed line representing the overall mean across all treatments
3. Adds gray dotted lines showing ±1 standard deviation from the overall mean
4. Provides a legend explaining the reference lines
5. Features clear labels for the title, x-axis, and y-axis

The boxplots themselves show the median (center line), interquartile range (box), and potential outliers (points) for each treatment, allowing for easy visual comparison between treatment effects.

The reference lines provide context about how each treatment compares to the overall distribution of the data, helping to identify treatments that produce above-average or below-average results.
:::
