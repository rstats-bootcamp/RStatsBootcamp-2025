...
For these exercises, run the code below to recreate the data object `pest`. There are 40 rows and two variables: `damage` and `treatment`.
...
## Solution: Visualizing Treatment Effects on Leaf Damage

```{r}
#| echo: true
#| eval: false

# Create a boxplot showing damage by treatment
boxplot(damage ~ treatment, data = pest,
        main = "Effect of Treatments on Leaf Damage",
        xlab = "Treatment Type",
        ylab = "Leaf Damage (mm²)",
        col = c("lightgray", "lightgreen", "lightblue", "lightyellow"))

# Add horizontal line for grand mean
abline(h = mean(pest$damage), 
       lty = 2, lwd = 2, col = "red")

# Add raw data points
set.seed(123)  # For reproducible jitter
points(x = jitter(as.numeric(factor(pest$treatment)), amount = 0.1),
       y = pest$damage,
       pch = 19, col = "blue", cex = 0.8)

# Add mean points for each group
means <- tapply(pest$damage, pest$treatment, mean)
points(x = 1:4, y = means, 
       pch = 18, col = "red", cex = 2)

# Add legend
legend("topright", 
       legend = c("Individual observations", "Group mean", "Grand mean"),
       pch = c(19, 18, NA),
       lty = c(NA, NA, 2),
       col = c("blue", "red", "red"),
       pt.cex = c(0.8, 2, NA),
...
The boxplot reveals clear trends in the data:

1. The control group shows the highest leaf damage, with a median around 100 mm².
2. The x.full treatment appears most effective at reducing damage, with values clustering around 50 mm².
3. The x.half treatment shows intermediate effectiveness.
4. The organic treatment shows damage levels closer to the control.

Initial assessment of ANOVA assumptions:
- The spread (variance) appears roughly similar across treatments, suggesting homoscedasticity.
- The distributions within each treatment look approximately symmetric, with no extreme outliers.
- Each box contains 10 observations, providing adequate sample size per group.
- There's clear separation between some groups, suggesting potential significant differences.

The visualization supports proceeding with formal assumption testing for ANOVA.
:::
...
::: {.callout-tip collapse="true"}
## Solution: Testing Gaussian Residuals Assumption
...
The graphical assessment of residuals shows:

1. The histogram of residuals appears roughly bell-shaped, with a slight right skew but no extreme departures from normality.

2. The Q-Q plot shows that most points fall along the reference line, with minor deviations at the tails. This suggests the residuals are approximately normally distributed.

3. The Shapiro-Wilk test (W = 0.97, p = 0.38) indicates no significant departure from normality, as the p-value is well above the conventional threshold of 0.05.

Based on both visual inspection and formal testing, we can conclude that the residuals adequately meet the Gaussian assumption required for ANOVA. The slight deviations observed are not severe enough to invalidate the use of parametric ANOVA for this dataset.
:::
...
::: {.callout-tip collapse="true"}
## Solution: Testing Homoscedasticity Assumption
...
The assessment of homoscedasticity shows:

1. The plot of residuals versus fitted values shows no clear pattern or funnel shape, suggesting consistent variance across the range of fitted values.

2. The boxplot of residuals by treatment group shows similar spread (box width and whisker length) across all four treatments, indicating similar variances.

3. The Bartlett test (K-squared = 1.89, df = 3, p = 0.60) confirms that there is no significant difference in variances between the treatment groups, as the p-value is well above 0.05.

Based on both visual inspection and formal testing, we can conclude that the data meet the homoscedasticity assumption required for ANOVA. The variances appear to be sufficiently homogeneous across treatment groups to proceed with parametric analysis.
:::
...
::: {.callout-tip collapse="true"}
## Solution: One-way ANOVA Analysis
...
The one-way ANOVA reveals a significant effect of treatment type on leaf damage (F(3, 36) = 32.82, p < 0.001). This indicates that at least one treatment differs significantly from the others in its effect on leaf damage.

The mean damage values for each treatment were:
- Control: 104.0 mm²
- X.half: 71.3 mm²
- X.full: 48.2 mm²
- Organic: 97.3 mm²

The eta-squared value of 0.73 indicates that approximately 73% of the total variance in leaf damage is explained by the treatment factor, representing a large effect size.

These results strongly suggest that the pesticide treatments have differential effects on reducing leaf damage from red lily beetles. The x.full treatment appears most effective at reducing damage, followed by x.half, while the organic treatment shows damage levels closer to the control. Further post-hoc testing is needed to determine which specific treatments differ significantly from each other.
:::
...
::: {.callout-tip collapse="true"}
## Solution: Post-hoc Analysis of Treatment Effects
...
1. **Organic pesticide effectiveness**: The comparison between organic and control treatments shows no significant difference (diff = -6.7 mm², p = 0.56). This indicates that the organic pesticide is not effective at reducing leaf damage compared to no treatment.

2. **Dose effect**: The comparison between x.half and x.full treatments shows a significant difference (diff = 23.1 mm², p < 0.001). This indicates that dose does matter, with the full-strength non-organic pesticide being significantly more effective than the half-strength version.

Other significant findings:
- Both x.half and x.full treatments resulted in significantly less damage than the control (p < 0.001).
- Both x.half and x.full treatments resulted in significantly less damage than the organic treatment (p < 0.001).

The results can be summarized with significance groups:
- Group A: Control and Organic (not significantly different from each other)
- Group B: X.half (significantly different from all other treatments)
- Group C: X.full (significantly different from all other treatments)

These findings suggest that while conventional pesticides are effective at reducing leaf damage from red lily beetles, with higher doses providing better protection, the organic alternative tested here does not provide significant benefits compared to using no treatment at all.
:::
...
::: {.callout-tip collapse="true"}
## Solution: Iris Dataset ANOVA Question

A plausible practice question would be:

"Using the built-in iris dataset in R, investigate whether there are significant differences in sepal width among the three iris species (setosa, versicolor, and virginica). Follow these steps:

1. Create an appropriate visualization that shows the distribution of sepal width for each species.
2. Test the assumptions of ANOVA (normality of residuals and homoscedasticity).
3. Perform a one-way ANOVA to determine if there are significant differences in sepal width among species.
4. If significant differences are found, conduct appropriate post-hoc tests to determine which specific species differ from each other.
5. Calculate and interpret an appropriate effect size measure.
6. Report your findings in the technical style, including a clear interpretation of the biological significance of your results.

Your answer should include all necessary code, appropriate visualizations, and thorough interpretation of the statistical results."
...
# Add raw data points
set.seed(123)
points(x = jitter(as.numeric(iris$Species), amount = 0.1),
       y = iris$Sepal.Width,
       pch = 19, col = "darkgray", cex = 0.7)

# Add mean points
species_means <- tapply(iris$Sepal.Width, iris$Species, mean)
points(1:3, species_means, pch = 18, col = "red", cex = 1.5)

# 2. Test assumptions
iris_model <- aov(Sepal.Width ~ Species, data = iris)

# Normality of residuals
shapiro.test(residuals(iris_model))
...
anova_summary <- summary(iris_model)
SS_species <- anova_summary[[1]][1, "Sum Sq"]
SS_total <- sum(anova_summary[[1]][, "Sum Sq"])
eta_squared <- SS_species / SS_total
eta_squared
```

This question tests students' ability to:
1. Apply ANOVA in a biological context
2. Create informative visualizations
3. Test and interpret statistical assumptions
4. Perform and interpret post-hoc tests
5. Calculate and interpret effect sizes
6. Communicate statistical results in scientific format
:::
