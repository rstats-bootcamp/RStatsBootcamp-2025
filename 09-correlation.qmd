...
# Load the MASS package which contains the waders dataset
library(MASS)

# Load the waders data
data(waders)
...
# Step 2: Graph
# Create a combined visualization using ggplot2
ggplot(seals) +
  geom_point(aes(x = weight, y = heart, color = "Heart Mass"), size = 3, alpha = 0.7) +
  geom_smooth(aes(x = weight, y = heart, color = "Heart Mass"), 
              method = "lm", se = TRUE, linetype = 1) +
  geom_point(aes(x = weight, y = lung/10, color = "Lung Mass (scaled)"), size = 3, alpha = 0.7) +
  geom_smooth(aes(x = weight, y = lung/10, color = "Lung Mass (scaled)"), 
              method = "lm", se = TRUE, linetype = 2) +
  scale_y_continuous(
    name = "Heart Mass (g)",
    sec.axis = sec_axis(~.*10, name = "Lung Mass (g)")
  ) +
  scale_color_manual(values = c("Heart Mass" = "darkred", "Lung Mass (scaled)" = "darkblue")) +
  labs(
    title = "Relationship of Body Weight with Heart and Lung Mass",
    subtitle = "Cape Fur Seals (Arctocephalus pusillus)",
    x = "Body Weight (kg)",
    color = "Organ"
  ) +
  theme_minimal() +
...
# Load psych package for comparing correlations
# If not installed, use: install.packages("psych")
if (!require(psych)) {
  # If package is not available, calculate manually
  ...
  # Standard error
  se_diff <- sqrt((1 / (n - 3)) * (2 * (1 - r23^2) * (1 - r12^2 - r13^2 - r23^2) + r23^2 * (r12^2 + r13^2)))
  
  # Test statistic
  z <- (z12 - z13) / se_diff
  p_value <- 2 * (1 - pnorm(abs(z)))
  
  cat("Manual comparison of dependent correlations:\n")
  cat("z =", z, ", p-value =", p_value, "\n")
} else {
  # Use psych package if available
  library(psych)
  r.test(n = nrow(seals), r12 = cor_weight_heart$estimate, 
         r13 = cor_weight_lung$estimate, r23 = cor_heart_lung$estimate)
}
...
plot(lm(heart ~ weight, data = seals), which = 1, main = "Weight-Heart Residuals")
plot(lm(lung ~ weight, data = seals), which = 1, main = "Weight-Lung Residuals")
par(mfrow = c(1, 1))

# Create allometric scaling coefficients (log-log relationship)
log_model_heart <- lm(log(heart) ~ log(weight), data = seals)
log_model_lung <- lm(log(lung) ~ log(weight), data = seals)

# Display scaling coefficients
summary(log_model_heart)
summary(log_model_lung)
```
